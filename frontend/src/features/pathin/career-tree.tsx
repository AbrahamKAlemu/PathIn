...
export function CareerTree({
  ...
  const [horizontalNavigation, setHorizontalNavigation] = useState<
    HorizontalNavigation | undefined
  >(initialMap.navigator?.horizontalNavigation);
  ...
  function selectNode(
    ...
    void refreshHorizontalNavigation(nodeId);
  }

  async function refreshHorizontalNavigation(nodeId: string) {
    ...
    try {
      const response = await fetch(
        `${apiUrl.replace(/\/$/, "")}/api/v1/navigator/nodes/${nodeId}/expand`,
        {
          method: "POST",
          ...
          body: JSON.stringify({
            profile: {
              ...
              interests: profileDraft.interests
                ...
                .map((value) => value.trim())
                .filter(Boolean),
              locationPreferences: [profileDraft.location].filter(Boolean),
              goals: [profileDraft.goals].filter(Boolean),
            },
            activePathIds,
          }),
        },
      ...
      if (payload.horizontalNavigation) {
        setHorizontalNavigation(payload.horizontalNavigation);
      }
    } catch {
      // Keep the last statistically ranked branches when the API is unavailable.
    }
  }

  function selectHorizontalBranch(
    branchNodeId: string,
    branchLabel: string,
    side: "left" | "right",
  ) {
    if (!nodeById.has(branchNodeId)) {
      setStatusMessage(
        `${branchLabel} is a statistically ranked branch preview. Full node data is not loaded yet.`,
      );
      return;
    }

    const matchingPath = activePaths.find((path) =>
      path.nodeIds.includes(branchNodeId),
    );
    selectAndFocusNode(branchNodeId, matchingPath?.id);
    setStatusMessage(
      `Moved ${side} to ${branchLabel} using cohort-weighted branch ranking.`,
    );
  }
  ...
  function handleNodeKeyDown(
    ...
    if (event.key === "ArrowLeft" && horizontalNavigation?.left[0]) {
      event.preventDefault();
      const branch = horizontalNavigation.left[0];
      selectHorizontalBranch(branch.nodeId, branch.label, "left");
      return;
    }

    if (event.key === "ArrowRight" && horizontalNavigation?.right[0]) {
      event.preventDefault();
      const branch = horizontalNavigation.right[0];
      selectHorizontalBranch(branch.nodeId, branch.label, "right");
    }
  }

  const leftBranch = horizontalNavigation?.left[0] ?? null;
  const rightBranch = horizontalNavigation?.right[0] ?? null;
            ...
            <div className={styles.mapContext}>
              ...
              {mode === "build" ? (
                ...
                <div className={styles.scenarioStatus}>
                  ...
                  <small>
                    Use the left and right branch controls to compare
                    statistically ranked alternatives at this step.
                  </small>
                </div>
              )}
            </div>
                ...
                <div className={styles.focusNavigator}>
                  ...
                  <div className={styles.focusCenterRow}>
                    <button
                      aria-label={
                        leftBranch
                          ? `Move focus left to ${leftBranch.label}`
                          : "No statistically ranked left branch"
                      }
                      className={styles.lateralBranch}
                      disabled={!leftBranch}
                      onClick={() =>
                        leftBranch
                          ? selectHorizontalBranch(
                              leftBranch.nodeId,
                              leftBranch.label,
                              "left",
                            )
                    ...
                    >
                      ...
                      {leftBranch ? (
                        <>
                          <strong>{leftBranch.label}</strong>
                          <small>
                            {Math.round(leftBranch.confidence * 100)}% cohort fit
                          </small>
                        </>
                      ) : null}
                    </button>
                    ...
                    <button
                      aria-label={
                        rightBranch
                          ? `Move focus right to ${rightBranch.label}`
                          : "No statistically ranked right branch"
                      }
                      className={styles.lateralBranch}
                    ...
                    >
                      {rightBranch ? (
                        <>
                          <strong>{rightBranch.label}</strong>
                          <small>
                            {Math.round(rightBranch.confidence * 100)}% cohort fit
                          </small>
                        </>
                      ) : null}
                      <MiniIcon name="arrow-right" />
